...
void RepairCenter::initialize() {
  ...
  // Register the statistics signal to track the number of failed servers over
  // time. 'registerSignal' returns a handle used for efficient 'emit' calls.
  failedServersSignal = registerSignal("failedServers");

  // Emit initial value (0) so the statistics start recording from simulation
  // time 0.
...
void RepairCenter::handleMessage(cMessage *msg) {
  ...
  if (msg->isSelfMessage()) {
    ...
    // We stored it earlier in startRepair() to remember which server was being
    // fixed.
    RepairRequest *req = (RepairRequest *)msg->getContextPointer();

    // 3. Notify the Server that it is working again.
    // We send a response back through the 'out' gate at the index matching the
    // server ID.
    ...
    // 5. Memory Cleanup: Delete both the request and the finished timer
    // message.
    delete req;
    delete msg;

    // 6. Check the Queue: If other servers are waiting, take the next one
    // immediately.
    if (!queue.isEmpty()) {
  ...
  // CASE B: INCOMING MESSAGE (A new server has just failed)
    ...
    if (busyProgrammers < numProgrammers) {
      ...
      // No, all programmers are busy. Put the request into the waiting line
      // (FIFO).
      queue.insert(msg);
    }
  }
...
void RepairCenter::startRepair(cMessage *reqMsg) {
  ...
  // This allows us to "remember" the server ID and repair data when the timer
  // expires.
