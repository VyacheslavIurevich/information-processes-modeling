...
// Use the standard OMNeT++ namespace to avoid prefixing everything with 'omnetpp::'
...
class RepairCenter : public cSimpleModule {
  private:
    ...
    // The waiting line: A built-in OMNeT++ container that stores messages (RepairRequests)
    // when all programmers are busy. It automatically handles FIFO logic.
    cQueue queue;
    ...
    // Used to "publish" updates about the failed server count to the statistics engine.
  ...
  protected:
     ...
     * 2. If it's a self-message -> finishes a repair and checks the queue for next.
     */
