...
const double P_PARTIAL_ADJUSTED = PROB_PARTIAL_TOTAL / (1.0 - PROB_ATTACK_SUCCESS);
        ...
        if (isWorking) {
            ...
            if (p >= PROB_ATTACK_SUCCESS) { 
                ...
                if (uniform(0, 1) < P_PARTIAL_ADJUSTED) {
                    ...
                    // uniform() generates a random duration between PARTIAL_REPAIR_MIN and PARTIAL_REPAIR_MAX hours.
                    req->setRepairDuration(uniform(PARTIAL_REPAIR_MIN, PARTIAL_REPAIR_MAX) * SECONDS_IN_HOUR); 
                } else {
                    // Full failure branch (Remaining 60% of successful attacks)
                    // uniform() generates a random duration between FULL_REPAIR_MIN and FULL_REPAIR_MAX hours.
                    req->setRepairDuration(uniform(FULL_REPAIR_MIN, FULL_REPAIR_MAX) * SECONDS_IN_HOUR);
                }
